	  ...
	  <>
		<div className="select-none align-bottom text-left transform transition-all sm:align-middle transition flex divide-x divide-gray-300 -mb-1">
		  {prompts.map((prompt, index) => (
			<div key={index} className={`shadow-md hover:relative hover:shadow-2xl bg-black  transition py-4 px-4 pr-8 rounded-t-md inline-flex text-${currentPrompt === index  ? "gray-800" : "gray-600 shadow-3xl"} font-medium border-b-2 border-${currentPrompt === index ? "white" : "gray-200"} bg-${currentPrompt === index ? "white" : "gray-300"} hover:bg-${currentPrompt === index  ? "white" : "gray-100"} cursor-pointer`} onClick={()=>onChange(index)}>
